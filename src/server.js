...
// Find the build directory - look for it relative to src folder
...
console.log(`Files in build path:`, fs.existsSync(buildPath) ? fs.readdirSync(buildPath) : 'Directory not found');
...
app.get('*', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.status(404).send(`Flutter app not found. Build directory: ${buildPath}. Current working directory: ${process.cwd()}`);
  }
});
